...
def sigint_handler(signal, frame):
    print("\nMinecraft server creation aborted.")
    sys.exit(0)
...
generator_presets = {
    "classic": { "biome": "minecraft:plains", "lakes": False, "features": False, "structures": ["village"], "stronghold": False, "layers": [{ "block":"minecraft:bedrock", "height": 1 }, { "block": "minecraft:dirt", "height": 2 }, { "block": "minecraft:grass_block", "height": 1 }] },
    
    "tunnelers_dream": { "biome": "minecraft:mountains", "lakes": False, "features": True, "structures": ["mineshaft"], "stronghold": True, "layers": [{ "block": "minecraft:bedrock", "height": 1 }, { "block": "minecraft:stone", "height": 230 }, { "block": "minecraft:dirt", "height": 5 }, { "block": "minecraft:grass_block", "height": 1 }] },
    
    "water_world": { "biome": "minecraft:deep_ocean", "lakes": False, "features": False, "structures": ["monument", "ocean_ruin", "shipwreck"], "stronghold": False, "layers": [{ "block": "minecraft:bedrock", "height": 1 }, { "block": "minecraft:stone", "height": 5 }, { "block": "minecraft:dirt", "height": 5 }, { "block": "minecraft:sand", "height": 5 }, { "block": "minecraft:water", "height": 90 }] },
    
    "overworld": { "biome": "minecraft:plains", "lakes": True, "features": True, "structures": ["mineshaft", "pillager_outpost", "ruined_portal", "village"], "stronghold": True, "layers": [{ "block": "minecraft:bedrock", "height": 1 }, { "block": "minecraft:stone", "height": 59 }, { "block": "minecraft:dirt", "height": 3 }, { "block": "minecraft:grass_block", "height": 1 }] },
    
    "snowy_kingdom": { "biome": "minecraft:snowy_tundra", "lakes": False, "features": False, "structures": ["igloo", "village"], "stronghold": False, "layers": [{ "block": "minecraft:bedrock", "height": 1 }, { "block": "minecraft:stone", "height": 59 }, { "block": "minecraft:dirt", "height": 3 }, { "block": "minecraft:grass_block", "height": 1 }, { "block": "minecraft:snow", "height": 1 }] },
    
    "bottomless_pit": { "biome": "minecraft:plains", "lakes": False, "features": False, "structures": ["village"], "stronghold": False, "layers": [{ "block": "minecraft:cobblestone", "height": 2 }, { "block": "minecraft:dirt", "height": 3 }, { "block": "minecraft:grass_block", "height": 1 }] },
    
    "desert": { "biome": "minecraft:desert", "lakes": False, "features": True, "structures": ["desert_pyramid", "mineshaft", "village"], "stronghold": True, "layers": [{ "block": "minecraft:bedrock", "height": 1 }, { "block": "minecraft:stone", "height": 3 }, { "block": "minecraft:sandstone", "height": 52 }, { "block": "minecraft:sand", "height": 8 }] },
    
    "redstone_ready": { "biome": "minecraft:desert", "lakes": False, "features": False, "structures": [], "stronghold": False, "layers": [{ "block": "minecraft:bedrock", "height": 1 }, { "block": "minecraft:stone", "height": 3 }, { "block": "minecraft:sandstone", "height": 52 }] },
    
    "the_void": { "biome": "minecraft:the_void", "lakes": False, "features": True, "structures": [], "stronghold": False, "layers": [{ "block": "minecraft:air", "height": 1 }] }
}

plugin_offers = {
    "EssentialsX": { "id": 9089 },
    "Fast Async WorldEdit": { "id": 13932, "file_name": "FastAsyncWorldEdit", "additional_info": "Normal WorldEdit may be chosen later." },
    "LuckPerms": { "id": 28140 },
        ...
        if state == 0:  # on first trigger, build possible matches
            if text:  # cache matches (entries that start with entered text)
                if "," in text:
                    ...
                    self.matches = [prefix + s for s in self.options 
...
def completeWithComma(completions):
...
# Java executable
index += 1
print(f"{prefix()} State the path to your Java executable. If it's on the PATH variable, you can also just state the command.")
print(f"{Indent(0)}How to choose the Java version:\n{Indent(0)}- Below or equal to 1.16: Java 8 to Java 16, recommended: Java 8 or Java 11\n{Indent(0)}- 1.17: Java 16\n{Indent(0)}- 1.18: Java 17")
while True:
    java = input("> ")
    if which(java) is not None:
        ...
        if match is not None:
            print(f"{Indent(0)}Java version successfully detected: {match.group(1)}")
            break
        else:
            print(f"{Indent(0)}Could not detect Java version. Do you want to continue? (y/N)")
            ignore = False
            while True:
                ...
                if answer is not None:
                    if answer:
                        ...
                        print(f"{Indent(0)}State the path to your Java executable.")
                    break
            if ignore:
                break
    else:
        print(f"{Indent(0)}Executable not found.")
    ...
    try:
        ...
        if port > 0:
            if port <= 65535:
                ...
                print(f"{Indent(0)}The port has to be below 65535.")
        else:
            print(f"{Indent(0)}The port has to be above 0.")
    except ValueError:
        print(f"{Indent(0)}Please state a number.")
...
while True:
    ...
    if len(target_folder) > 0:
        ...
        if target_folder_path.is_file():
            print(f"{Indent(0)}Please provide a folder.")
        elif target_folder_path.is_dir() and len(os.listdir(target_folder)) > 0:
            print(f"{Indent(0)}Please provide an empty folder.")
        else:
            break
    else:
        print(f"{Indent(0)}Please provide a folder.")
...
while True:
    ...
    if paper_version in versions:
        ...
        print(f"{Indent(0)}Unknown version. Please choose from {', '.join(versions)}")
complete([])
        ...
        if screen is not None:
            if screen:
                ...
                while True:
                    screen_name = input("> ")
                    if " " in screen_name:
                        print(f"{Indent(1)}The screen name may not contain spaces.")
                    elif not screen_name:
                        print(f"{Indent(1)}Please provide a screen name.")
                    else:
                        break
...
# Minimum ram
index += 1
print(f"{prefix()}1 Choose a minimum ram usage.\n{Indent(1)}Please use the format as for the \"java\" commnad. E.g. \"1024K\", \"2048M\" or \"1G\"\n{Indent(1)}Leave empty if you don't want to specify a minimum ram usage.")
while True:
    minimum_ram = input("> ")
    if not minimum_ram:
        minimum_ram = None
        ...
        print(f"{Indent(1)}Incorrect ram specification")

# Maximum ram
print(f"{prefix()}2 Choose a maximum ram usage.\n{Indent(1)}Same format as in {str(index)}.1\n{Indent(1)}Leave empty if you don't want to specify a maximum ram usage.")
while True:
    maximum_ram = input("> ")
    if not maximum_ram:
        maximum_ram = None
...
# MOTD
index += 1
print(f"{prefix()} Choose a MOTD. You can use color codes with \"\u00A7\" and you can seperate lines using \"\\n\".\n{Indent(0)}Leave empty for default MOTD.")
answer = input("> ")
if answer:
    motd = ""
    for c in answer:
...
while True:
    ...
    if whitelist is not None:
        if whitelist:
            ...
            print(f"{prefix()}1 Which players should be whitelisted? Seperate with comma or enter nothing for nobody.")
            while True:
                already_added = []
                whitelist_players = []
                failed = False
                ...
                if len(answer) > 0:
                    ...
                    for player in players:
                        ...
                        if response is not None:
                            ...
                            print(f"{Indent(1)}Player \"{player.strip()}\" does not exist")
                            failed = True
                if not failed:
                    break
            break
    ...
    if len(answer) > 0:
        ...
        for player in players:
            ...
            if response is not None:
                ...
                print(f"{Indent(0)}Player \"{player.strip()}\" does not exist")
                failed = True
    if not failed:
        break
        ...
        if answer:
            ...
            def ask_gamerule(key, info, sub_index, i):
                if paper_version_parsed >= version.parse(info["since"]):
                    ...
                    print(f"{prefix()}{str(sub_index)}.{str(i)} Set gamerule \"{key}\" ({'true/false y/n' if info['type'] == 'bool' else info['type']})\n{iindent}Defaults to \"{info['default']}\"")
                    if info["type"] == "bool":
                        complete(["true", "false"])
        ...
        if specific_settings:
            ...
            while True:
                ...
                if not level_name:
                    ...
                    print(f"{Iindent()}The level name may not contain spaces.")
                else:
                    break
            ...
            print(f"{prefix()}{str(i)} Choose a level type ({'/'.join(types)})\n{Iindent()}Defaults to \"default\".")
            complete(types)
            while True:
                ...
                if level_type in types:
                    ...
                    print(f"{Iindent()}Unknown level type.")
            
            # Generation settings
            generator_settings = None
            if level_type == "flat":
                i += 1
                print(f"{prefix()}{str(i)} Do you want to change the generator settings? (y/N)\n{Iindent()}Warning: This feature has only been tested in 1.17+")
                while True:
                    ...
                    if answer is not None:
                        if answer:
                            ...
                            print(f"{prefix()}{str(i)}.1 Choose a preset or \"custom\" to customize values. Defaults to \"classic\".\n{Iindent()}  Presets: {', '.join(presets)}")
                            complete(presets)
                            while True:
                                answer = input("> ").lower()
                                if answer in presets:
                                    ...
                                    if answer != "custom":
                                        ...
                                        # Biome
                                        print(f"{prefix()}{str(i)}.2 Choose a biome id. Defaults to \"plains\".\n{Iindent()}  For a list of biome ids, see https://minecraft.fandom.com/wiki/Biome/ID")
                                        complete(biomes)
                                        while True:
                                            answer = input("> ").lower()
                                            if not answer:
                                                ...
                                                if answer in biomes:
                                                    ...
                                                    print(f"{Iindent()}  Unknown biome.")
                                        
                                        # Features
                                        print(f"{prefix()}{str(i)}.3 Do you want biome-specific features to generate? (y/N)")
                                        while True:
                                            ...
                                            if not answer:
                                                ...
                                                for structure in answer.split(","):
                                                    ...
                                                    if structure in generator_structures:
                                                        ...
                                                        failed = True
                                                        print(f"{Iindent()}  Unknown structure: \"{structure}\"")
                                                        break
                                                if not failed:
                                                    structures["structures"] = sub_structures
                                                    break
                                        ...
                                        # Stronghold
                                        print(f"{prefix()}{str(i)}.6 Do you want strongholds to generate? (y/N)\n{Iindent()}  They may not generate. It's buggy.")
                                        while True:
                                            answer = parse_yes_no(input("> "), False, iindent() + 2)
                                            if answer is not None:
                                                if answer:
                                        ...
                                        while True:
                                            ...
                                            if not answer:
                                                if not layers:
                                                    print(f"{Iindent()}  Specify at least 1 layer.")
                                                else:
                                                    break
                                            else:
                                                ...
                                                if len(answers) > 1:
                                                    try:
                                                        ...
                                                        print(f"{Iindent}  \"{answers[1]}\" is not a number.")
                                                        continue
                                                else:
                                                    height = 1
                                    ...
                                    print(f"{Iindent()}  Unknown preset.")
                        break
            
            # Seed
            i += 1
            print(f"{prefix()}{str(i)} Choose a world seed. (int)\n{Iindent()}Leave empty for a random seed.")
            answer = input("> ")
            ...
            if paper_version_parsed < version.parse("1.17"):
                i += 1
                print(f"{prefix()}{str(i)} Choose a max build height. (int)\n{Iindent()}Maximum is 256. Has to be a multiple of 8.\n{Iindent()}Defaults to 256.")
                while True:
                    answer = input("> ")
                    if not answer:
                        ...
                        if max_build_height >= 1:
                            if max_build_height <= 256:
                                if max_build_height % 8 == 0:
                                    break
                                print(f"{Iindent()}The max build height has to be a multiple of 8.")
                            else:
                                print(f"{Iindent()}The max build height has to be below or equal to 256.")
                        else:
                            print(f"{Iindent()}The max build height has to be above 0.")
                    except ValueError:
                        print(f"{Iindent()}Please state a number.")
            else:
                max_build_height = None
            ...
            while True:
                ...
                try:
                    ...
                    if view_distance >= 3 and view_distance <= 32:
                        break
                    else:
                        print(f"{Iindent()}Please choose a value between 3 and 32.")
                except ValueError:
                    print(f"{Iindent()}Please state a number.")
            ...
            if paper_version_parsed >= version.parse("1.18"):
                ...
                while True:
                    ...
                    try:
                        ...
                        if simulation_distance >= 5 and simulation_distance <= 32:
                            break
                        else:
            ...
            if paper_version_parsed >= version.parse("1.16"):
                i += 1
                print(f"{prefix()}{str(i)} Choose an entity broadcast range percentage. (10-1000)\n{Iindent()}Defaults to 100.")
                while True:
                    answer = input("> ")
                    if not answer:
                    ...
                    try:
                        ...
                        if entity_broadcast_range_percentage >= 10 and entity_broadcast_range_percentage <= 1000:
                            break
                        else:
                            print(f"{Iindent()}Please choose a value between 10 and 1000.")
                    except ValueError:
                        print(f"{Iindent()}Please state a number.")
            else:
                entity_broadcast_range_percentage = None
            ...
            while True:
                ...
                if gamemode in gamemodes:
                    ...
                    print(f"{Iindent()}Unknown gamemode")
            complete([])
            ...
            if paper_version_parsed >= version.parse("1.14"):
                ...
                default = "1"
            print(f"{prefix()}{str(i)} Choose a difficulty ({'/'.join(difficulties)})\n{Iindent()}Defaults to \"{default}\".")
            ...
            while True:
                ...
                if difficulty in difficulties:
                    ...
                    print(f"{Iindent()}Unknown difficulty")
            complete([])
...
while True:
    ...
    if change_patches is not None:
        if change_patches:
            # Discount exploit
            print(f"{prefix()}1 Do you want to enable \"fix-curing-zombie-villager-discount-exploit\"? (Y/n)\n{Indent(1)}If enabled, villagers won't give you extrem discounts after curing.")
            while True:
            ...
            print(f"{prefix()}2 Do you want to enable \"allow-permanent-block-break-exploits\"? (y/N)\n{Indent(1)}If enabled, you can remove bedrock using piston glitches.")
            while True:
            ...
            # Piston duplication
            print(f"{prefix()}2 Do you want to enable \"allow-piston-duplication\"? (y/N)\n{Indent(1)}If enabled, you can duplicate e.g. carpets using pistons.")
            while True:
            ...
            for plugin, info in plugin_offers.items():
                ...
                if "incompatible" in info:
                    ...
                    if incompatible_plugins:
                        i += 1
                        print(f"{prefix()}{str(i)} The plugin \"{plugin}\" is incompatible with the plugin{'s' if len(incompatible_plugins) != 1 else ''} \"{delimiter.join(incompatible_plugins)}\".")
                        continue
                ...
                if "depends" in info:
                    ...
                    if needed_plugins:
                        i += 1
                        print(f"{prefix()}{str(i)} The plugin \"{plugin}\" depends on the plugin{'s' if len(needed_plugins) != 1 else ''} \"{delimiter.join(needed_plugins)}\".")
                        continue
                ...
                if "id" not in info:
                    ...
                    i += 1
                    print(f"{prefix()}{str(i)} Install the plugin \"{plugin}\"? (y/N)\n{Iindent}No further information due to the plugin not being available on spigotmc.org")
                    if "additional_info" in info:
                        print(Iindent + info["additional_info"])
                    while True:
                        answer = parse_yes_no(input("> "), False, iindent)
                        if answer is not None:
                            if answer:
                                plugins[plugin] = { "url": info["url"], "file": (info["file_name"] if "file_name" in info else plugin) + ".jar" }
                            break
                    continue
                ...
                if len(response) > 0:
                    ...
                    print(f"{prefix()}{str(i)} Install the plugin \"{plugin}\"?{'' if tested else ' [Not tested in this version!]'} (y/N)\n{Iindent}{spiget_info['tag']}")
                    if "additional_info" in info:
                        print(Iindent + info["additional_info"])
                    while True:
                        answer = parse_yes_no(input("> "), False, iindent)
                        if answer is not None:
                            if answer:
                                ...
                                if plugin == "EssentialsX":
                                    latest_version = json.loads(request.urlopen(f"https://api.spiget.org/v2/resources/{str(info['id'])}/versions/latest").read())["name"]
                                    url = f"https://github.com/EssentialsX/Essentials/releases/download/{latest_version}/EssentialsX-{latest_version}.jar"
                                elif plugin == "Fast Async WorldEdit":
                                    console = request.urlopen("https://ci.athion.net/job/FastAsyncWorldEdit-1.17/lastSuccessfulBuild/consoleText").read()
                                    match = re.search("FastAsyncWorldEdit-Bukkit-(\\d|\\.)+-\\d+\\.jar", str(console))
                                    if match is not None:
                                        url = f"https://ci.athion.net/job/FastAsyncWorldEdit-1.17/lastSuccessfulBuild/artifact/artifacts/{match.group()}"
                                else: 
                                    url = info["url"] if "url" in info else f"https://api.spiget.org/v2/resources/{str(info['id'])}/download"
                                
                                if url is not None:
                                    plugins[plugin] = { "url": url, "file": (info["file_name"] if "file_name" in info else plugin) + ".jar" }
                                else:
...
# Eula
print("\nType \"Yes\" to agree to the EULA of Minecraft.\nSee https://account.mojang.com/documents/minecraft_eula for more information.")
while True:
    if input("> ").lower() == "yes":
        break
    else:
...
latest_build = json.loads(request.urlopen(f"https://papermc.io/api/v2/projects/paper/versions/{paper_version}").read())["builds"][-1]
file_name = json.loads(request.urlopen(f"https://papermc.io/api/v2/projects/paper/versions/{paper_version}/builds/{latest_build}").read())["downloads"]["application"]["name"]
request.urlretrieve(f"https://papermc.io/api/v2/projects/paper/versions/{paper_version}/builds/{latest_build}/downloads/{file_name}", "Paper.jar")
...
if len(gamerules) > 0:
    ...
    with open("spigot.yml", "r+") as spigot_file:
        spigot_data = spigot_file.read().replace("restart-on-crash: false", "restart-on-crash: " + ("true" if not screen or not restart else str(restart_on_crash).lower()))
        spigot_file.seek(0)
